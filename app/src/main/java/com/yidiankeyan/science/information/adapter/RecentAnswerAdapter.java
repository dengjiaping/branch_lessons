 ...
 * -最新问答和偷听榜
 */
public class RecentAnswerAdapter extends BaseAdapter implements View.OnClickListener {

    private Context mContext;
    private LayoutInflater mInflater;
    private List<NewScienceHelp> list = new ArrayList<>();
    private String url = "http://f.hiphotos.baidu.com/image/h%3D200/sign=9d661b069dcad1c8cfbbfb274f3e67c4/5bafa40f4bfbfbed064c3d5670f0f736afc31f85.jpg";
    private OnEavesdropClick onEavesdropClick;

    public RecentAnswerAdapter(Context mContext, List<NewScienceHelp> list) {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            convertView = mInflater.inflate(R.layout.item_recent_answer, parent, false);
            ...
            holder.imgEavedrop = (AutoRelativeLayout) convertView.findViewById(R.id.img_eavedrop);
            holder.imgClick = (ImageView) convertView.findViewById(R.id.img_click);
            holder.imgAuthenticated = (ImageView) convertView.findViewById(R.id.img_authenticated);
            holder.imgPlay = (ImageView) convertView.findViewById(R.id.img_play);
            holder.tvOccupationalName = (TextView) convertView.findViewById(R.id.tv_occupational_name);
            holder.tvOneEavesdropping = (TextView) convertView.findViewById(R.id.tv_one_eavesdropping);
        ...
        if (DemoApplication.answerAlbumMap.get(scienceHelp.getKederDB().getKedaalbumid()) != null)
            holder.tvContent.setText("【" + DemoApplication.answerAlbumMap.get(scienceHelp.getKederDB().getKedaalbumid()).getKdname() + "】" + scienceHelp.getKederDB().getQuestions());
        else
...
//        else
        Glide.with(mContext).load(Util.getImgUrl(scienceHelp.getSolver().getCoverimg())).asBitmap().centerCrop().placeholder(R.drawable.icon_hotload_failed).error(R.drawable.icon_hotload_failed)
...
//        holder.tvOccupationalName.setText(scienceHelp.getPosition()+"");
        if (!TextUtils.isEmpty(scienceHelp.getSolver().getProfession()) && !TextUtils.equals("null", scienceHelp.getSolver().getProfession())) {
            holder.tvOccupationalName.setText(scienceHelp.getSolver().getProfession());
        }
        if (scienceHelp.getSolver().getAuthenticated() == 2) {
            holder.imgAuthenticated.setVisibility(View.VISIBLE);
        } else {
            holder.imgAuthenticated.setVisibility(View.GONE);
        }
        int length;
        if (DemoApplication.answerAlbumMap.get(scienceHelp.getKederDB().getKedaalbumid()) != null && !TextUtils.isEmpty(DemoApplication.answerAlbumMap.get(scienceHelp.getKederDB().getKedaalbumid()).getKdname())) {
            length = ("【" + DemoApplication.answerAlbumMap.get(scienceHelp.getKederDB().getKedaalbumid()).getKdname() + "】").length();
        } else {
            length = 0;
        }
        ...
        holder.imgClick.setTag(holder);
        holder.imgClick.setOnClickListener(this);
...
//                    EventBus.getDefault().post(msg);
    ...
    class ViewHolder {
        ...
        private ImageView imgAuthenticated;
