 ...
 * -音频已订专辑Adapter
 */
public class ClassAudiosAdapter extends BaseAdapter implements View.OnClickListener {
    ...
    public ClassAudiosAdapter(Context context, List<BookedAlbumBean> mDatas) {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        switch (allBean.getAlbumtype()) {
            case 1:
                ...
                break;
            case 2:
                viewHolder.imgNameType.setImageResource(R.drawable.animation_column_three);
                break;
            case 3:
                viewHolder.imgNameType.setImageResource(R.drawable.jc_play_normal);
                break;
        }

        viewHolder.txtContent.setText(allBean.getRecenttitle());
        viewHolder.txtTitle.setText(allBean.getAlbumname());
        viewHolder.txtName.setText("by " + allBean.getAuthorname());
        viewHolder.txtNumber.setText(allBean.getContentnum() + "篇");
        ...
        viewHolder.txtTou.setText(allBean.getReadnum() + "");
        ...
        switch (v.getId()) {
            case R.id.btn_xiala:
                ...
                if (mPopupWindow == null) {
                    ...
                    mPopupWindow.setOutsideTouchable(true);
                    mPopupWindow.setBackgroundDrawable(new BitmapDrawable());
                    mPopupWindow.showAtLocation(((Activity) context).findViewById(R.id.ll_class_audio), Gravity.BOTTOM, 0, 0);
                    mPopupWindow.setOnDismissListener(new PopupWindow.OnDismissListener() {
                        @Override
                        public void onDismiss() {
                            Util.finishPop((Activity) context, mPopupWindow);
...
//                      notifyDataSetChanged();
                ...
                mPopupWindow.getContentView().findViewById(R.id.ll_cancelsub).setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        finishPop(mPopupWindow);
                        ((ClassAudioActivity) context).showLoadingDialog("正在操作");
        ...
        HttpUtil.post(context, SystemConstant.URL + SystemConstant.ORDER_ABOLISH_ALBUM, map, new HttpUtil.HttpCallBack() {
            ...
            public void successResult(ResultEntity result) throws JSONException {
                if (200 == result.getCode()) {
                    mDatas.remove(viewHolder.position);
                    notifyDataSetChanged();
                    ((ClassAudioActivity) context).loadingDismiss();
                }
            }
