...
import fm.jiecao.jcvideoplayer_lib.proxy.utils.MediaPlayerProxy;
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case HANDLER_PREPARE:
                    try {
                        if (!new File(CURRENT_PLAYING_URL).exists()) {
                            startProxy();
                            CURRENT_PLAYING_URL = proxy.getProxyURL(CURRENT_PLAYING_URL);
                        }
                        currentVideoWidth = 0;
